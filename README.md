...
它不是要取代 Flickr，也不是把原圖保存到這裡。照片仍在 Flickr；這個專案建立的是一層「找得到、看得懂、能持續維護」的索引，讓籌備團隊可以依照社群貼文、網站主視覺、贊助提案、贊助成果、新聞稿與活動回顧等真實需求找照片。
...
![SITCON Flickr Photo Finder 角色定位圖：Flickr 是照片來源，Google Sheets 是正式索引，這個專案提供資料規則、匯入工具、AI 輔助與使用介面](docs/assets/readme/photo-finder-flow.svg)
...
- Google Sheets 是正式照片索引；`fixtures/*.csv` 只是本機範例與測試資料。
