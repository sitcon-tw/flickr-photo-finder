...
這會輸出到 `tmp/sheets-init/`，包含 `photos.csv`、`albums.csv`、`import_batches.csv`、`taxonomy.csv`、`sponsorship_items.csv` 與 `manifest.json`。建立正式 Google Sheets 時，請建立同名 tabs。SDK 寫入工具尚未實作前，可暫時把對應 CSV 匯入或貼上；後續應改由 repo CLI 透過官方 Google Sheets API SDK 套用。這個指令不會連線或寫入 Google Sheets。
