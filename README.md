...
目前 repo 已有本機搜尋 UI、資料驗證、SITCON Flickr 相簿盤點、可回寫 Google Sheets `albums` 的同步 CSV 產生工具、從正式 Sheets 匯出或直接從 Sheets API 讀取相簿清單後互動式選擇本次處理相簿的 CLI、可從選定相簿產生一次 intake run artifact 的工具、準備/檢查/比對/規劃並回寫 AI 初標候選 metadata 的本機工作流程，以及 GitHub Pages artifact build/deploy workflow。run artifact 會包含 Google Sheets `photos` 候選列、`albums.last_processed_at` 更新、`import_batches` 批次紀錄與摘要；AI metadata 更新流程會先產生可審核 diff 與 update plan，dry-run 後才可選擇寫入 Sheets，且寫入後會讀回驗證。正式 Google Sheets 表格讀寫的主要技術選擇是官方 Google Sheets API SDK；Apps Script source 與 `clasp` deploy 仍是目標流程。
