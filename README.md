...
`pnpm workflow` 會先說明完整資料流，再依任務標示目前階段、需要的輸入、會產生的輸出與下一步，協助使用者開始操作常見流程，例如檢查資料、處理一本 Flickr 相簿、準備 AI 初標工作包、建立 attempt、驗證 AI 初標結果、產生報表、執行 `visual_description` 搜尋實驗、操作 Google Sheets 工具或開本機搜尋 UI。下表中的低階指令仍保留給自動化、除錯與文件交叉引用。
...
| 建立並檢查 GitHub Pages artifact | `pnpm pages:build` 後接 `pnpm pages:check`；互動流程可用 `pnpm workflow -- --task pages-build` |
