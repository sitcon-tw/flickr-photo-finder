...
- `pnpm validate:data`，驗證本機照片與相簿測試資料。
- `pnpm albums:discover`，盤點 SITCON Flickr 公開相簿清單。
- `pnpm album:add -- <album-id-or-flickr-album-url>`，檢查或匯入單本相簿中尚未索引的照片。
- `pnpm photo:add -- <flickr-photo-url>`，從單張 Flickr 照片產生候選資料列。
