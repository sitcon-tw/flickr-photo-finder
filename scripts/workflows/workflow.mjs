...
async function buildPagesArtifact() {
  runPnpm("finder:build");
  runPnpm("finder:check");
  console.log("");
  console.log("下一步：");
...
async function runBulkAiLabeling() {
  ...
  if (await askYesNo("要重新準備 shard workspace？這會清掉同 run 的舊 shard inputs/prompts/outputs", true)) {
    const maxPhotosPerShard = await ask("每個 shard 最多幾張照片", "20");
...
async function runSheetsTools() {
  const choices = [
    ...
    ["sheets:onboarding:check", "唯讀檢查正式/練習表 onboarding 鏈"],
    ...
    ["apps-script:status", "檢查 production Apps Script source 與遠端差異"],
    ["apps-script:push", "推送 Apps Script source 到 production，不寫 Sheets 資料"],
