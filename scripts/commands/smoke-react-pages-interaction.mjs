...
async function runInteractionSmoke({ url }) {
  ...
  try {
    ...
    initialUrl.searchParams.set("task", "hero");
    ...
    if (hydratedValue.activeTaskLabel !== "網站橫幅") {
      throw new Error(`Expected URL task to hydrate active task mode, got ${hydratedValue.activeTaskLabel}`);
    }
    if (!String(hydratedValue.bodyText).includes("以年份新到舊顯示符合搜尋的照片")) {
      throw new Error("Expected hydrated sort context text");
    }
    ...
    if (updatedParams.get("scene") !== "攤位") {
      throw new Error(`Expected reset to preserve filter URL keys, got ${updatedValue.searchParams}`);
    }
