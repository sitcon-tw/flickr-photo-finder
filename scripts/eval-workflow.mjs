...
const tasks = [
  ...
  },
  {
    ...
    next: ["把 run 目錄中的 ai-labeling-prompt.md 與工作包交給模型；模型只輸出 metadata-proposals.json。若要多模型比較，接著建立 attempt。"],
  ...
  },
  {
    ...
    next: ["把 attempt 目錄中的 ai-labeling-prompt.md 與工作包交給對應模型；模型只輸出 metadata-proposals.json，之後由操作者執行 review。"],
...
function printEvalSummary() {
  ...
  console.log("3. eval:attempt 讓不同模型或輪次共用同一批輸入，方便公平比較。");
  console.log("4. 模型輸出仍用 ai:review 驗證，並可用 ai:report 產生單次或多模型報表。");
...
async function showEvalOverview() {
  ...
  console.log("- tmp/ai-runs/: AI run 與 attempt 工作包，包含 photos.json、images/ 與 metadata-proposals.json。");
