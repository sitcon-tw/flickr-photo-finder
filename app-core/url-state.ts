...
type FilterValues = FilterValue | FilterValue[];
...
type UrlStateInput = {
  ...
  selectedPhotoIds?: Iterable<FilterValue>;
};
...
/*! Generated app/url-state.js from app-core/url-state.ts; edit the TypeScript source. */
let filterUrlKeys: Record<string, string> = {
  album: "album",
  use: "use",
  mood: "mood",
  scene: "scene",
...
export function applyUrlStateRegistry(interfaceRegistry: InterfaceRegistry | null | undefined) {
...
function cleanValues(values: FilterValues): string[] {
...
function appendValues(params: URLSearchParams, key: string, values: FilterValues) {
  for (const value of cleanValues(values)) {
    params.append(key, value);
  }
}

export function encodeUrlState(state: UrlStateInput): URLSearchParams {
...
export function decodeUrlState(params: URLSearchParams): UrlStateOutput {
