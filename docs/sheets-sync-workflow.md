...
使用者通常會提供 Flickr 相簿 URL，例如：

```text
https://flickr.com/photos/sitcon/albums/72177720333438501/
```

建議流程：

1. 技術志工或 agent 使用 repo 工具掃描相簿。
2. 工具取得相簿中的照片 ID、照片 URL、縮圖 URL、相簿名稱與可用的 Flickr metadata。
3. 工具和 Google Sheets `photos` 既有 `photo_id` 比對，避免重複匯入。
4. 新照片以最低必要欄位寫入 `photos`。
5. 同步更新 `albums` 與 `import_batches`。
6. 新增照片預設 `curation_status = unreviewed`。
