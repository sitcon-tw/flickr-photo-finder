...
SDK 寫入工具尚未實作前，可以暫時把每個 CSV 匯入或貼到對應 tab。後續應改由 repo CLI 透過官方 Google Sheets API SDK 套用初始化資料。`manifest.json` 用來確認這批初始化檔的 schema version、來源設定與檔案對應，不需要匯入 Sheets。
...
確認沒有覆蓋風險後，正式 Sheets 表格寫入的主要技術選擇是官方 Google Sheets API SDK。repo 工具負責產生可檢查資料、schema、validation、風險檢查、dry-run 與寫入後驗證；不再把 rclone 或 Google Drive 檔案匯入當成主要 Sheets 寫入流程。
...
產出的 CSV 通過 validation 後，應由 SDK-based Sheets 寫入工具套用到正式 Google Sheets，或在 SDK 工具尚未實作前由人類暫時手動匯入。這一步不需要把正式 Sheets 資料 commit 回 repo。
...
現階段 repo 尚未實作 SDK 寫入工具，因此 `pnpm sheets:init` 仍只產生初始化 CSV，`pnpm sheets:check` 仍只做公開讀取與覆蓋風險檢查。下一步應實作 SDK-based `sheets:apply` 或等價指令，把初始化資料、`albums` 更新與 intake run artifact 以可確認方式寫入正式 Sheets。

rclone 若未來仍被使用，定位應限於 Google Drive 檔案備份、匯出檔搬運或組織既有檔案工作流，不作為 Sheets tab/range 寫入的主要流程。
...
不同維護者本機可用的授權方式不一定相同。此 repo 不應假設某個人的 Google 帳號、OAuth token cache、`gcloud`、瀏覽器登入狀態、`clasp` 權限或 `rclone` remote 能在其他人手上重現。
...
SDK credential、token cache、`rclone` remote 或其他個人授權設定不應 commit。若專案要記錄操作方式，只能記錄需要的能力、scope、檔案對應與驗證步驟，不記錄個人 token 或組織內部權限細節。

建議的可攜驗證順序：

1. 執行 `pnpm sheets:init`，確認初始化 CSV 可由 repo 產生。
2. 執行 `pnpm sheets:check`，確認公開 Sheets 固定 tabs 沒有覆蓋風險。
3. 執行 SDK 寫入工具的 dry-run，確認目標 spreadsheet、tabs、header、預計新增或更新列數。
4. 人類確認後才執行寫入。
5. 寫入後由 SDK 工具讀回驗證，並可再執行 `pnpm sheets:check` 確認 tabs 已有資料且 header 合理；此時出現 `has data` 是預期結果，不應再拿它判斷是否適合初始化。
