...
2. 檢查 Apps Script 綁定的 `GeneratedConfig.js` 是否和目前 repo schema / taxonomy 不一致。
...
若 `更新欄位選項` 執行後 `schema_meta` 是整張空白，這不是成功狀態。重新推送目前 repo 中的 Apps Script source 後再執行 refresh；目前 source 會在寫入後讀回檢查，若必要欄位仍空白應直接報錯。

若 `taxonomy` 的 `label_zh` 整欄空白，代表正式 Sheet 停在既有三欄格式或過期資料。可先用 `pnpm sheets:sync-taxonomy` 做 dry-run，確認後用 `pnpm sheets:sync-taxonomy -- --write` 重寫 `taxonomy` tab；也可以重新推送目前 repo 中的 Apps Script source 後執行 `更新欄位選項`。
...
若錯誤訊息要求目前 manifest 未列出的 scope，通常代表 Sheet UI 使用的 bound script source 與目前 repo 不一致，或本機 `.clasp.json` 綁到錯誤的 Apps Script 專案。請確認已重新執行 `pnpm apps-script:push`，且 Sheet UI 開啟的是同一份 bound script。
