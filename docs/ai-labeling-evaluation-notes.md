...
兩者都沒有 confidence。這是格式允許的選擇，但對人類排序與抽查不利；若未來要把 attempt 作為可操作的候選基底，prompt 應要求至少在高風險欄位穩定提供 confidence，或乾脆維持全省略並不要在報表中把 confidence 當排序訊號。

### Review notes

Claude 的 review warnings：

- 11 張照片有兒童、名牌、姓名、QR code、識別證或清晰臉部等公開使用風險線索，但未提供 `public_use_status`。
- 所有候選值都沒有 confidence。
- `54978186735` 被提示 `people_count = 0` 但 scene tags 或 reason 仍提到人物相關線索，需要人工確認。

GPT 的 review warnings：

- 12 張照片有公開使用風險線索但未提供 `public_use_status`。
- 所有候選值都沒有 confidence。

這輪可以看出目前 prompt 對 `public_use_status` 的「有風險才填」方向是正確的，但模型仍偏保守地省略風險提醒。尤其跨活動樣本含兒童、識別證、QR code 與具名文宣時，review focus 仍需要保留公開使用抽查入口。
