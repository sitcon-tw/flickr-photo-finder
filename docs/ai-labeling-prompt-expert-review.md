...
`pnpm eval:prompt-review` 是 prompt、schema、search 與 docs 變更前的決策 gate。它彙整既有 run evidence 與角色 review；owner 接受決策包後，才另開實作切片。

```mermaid
flowchart TD
  A["ai:review / ai:report / eval:search evidence"] --> B["eval:prompt-review prepare"]
  B --> C["input-manifest.json 記錄 prompt / schema / run hash"]
  B --> D["expert-prompts 指派角色 agent"]
...
本輪審查以 repo 內的專案文件、schema、taxonomy、prompt、AI 評估紀錄與 public frontend 代理研究為依據。審查者只做唯讀分析，不修改 repo 檔案。
...
這四個角色是 prompt review 的預設核心組，不是永久唯一角色。若後續主題集中在贊助履約、使用提醒、前端排序或特定活動類型，可在決策包中加入臨時審查角色。

### Review provenance

`eval:prompt-review prepare` 只產生 `expert-prompts/` 與空的 `expert-reviews/`，不會自動呼叫外部 LLM，也不會自動分派 agent。若決策需要獨立審查，操作者必須明確把 `expert-prompts/` 分派給不同 agent 或不同可追溯執行 session，並在回填的 `expert-reviews/` 中記錄 review provenance。

每份 `expert-reviews/*.md` 或 `*.json` 至少應交代：

- `reviewer_type`：`independent-agent` 或 `same-agent`。
- `reviewer_id` 與 `session_id`，讓後續維護者能追溯是哪個 agent 或哪次執行產生 review。
- 是否獨立閱讀 evidence。
- 是否與其他 review 共用同一個 agent context。

由同一個 agent 依多個角色撰寫的 review，或無法確認獨立性的多角色輸出，只能視為 `same-agent synthesis`，不應標示為獨立審查共識。這種 synthesis 可以用來快速建立初版決策包，但 owner 解讀時應把它視為單一審查者的多視角分析；若要作為高信心決策 gate，應改由不同 agent 或不同可追溯執行 session 各自產生 review 後再 compile。

接手維護者若看到 `expert-prompts/` 但還沒有 `expert-reviews/`，應先主動確認 owner 需要哪一種審查層級：獨立 agent review、不同可追溯執行 session，或 same-agent synthesis。不要在未確認的情況下直接由同一個 agent 補完所有角色並稱為獨立審查共識。
...
目前合約允許省略 confidence，但評估工具又會提醒缺少 confidence 不利人工排序。角色審查共識是：沒有校準資料前，不應讓模型零散輸出信心分數。
...
| 是否讓工具自動呼叫外部 LLM 審查者 | 不建議。provider、credential、成本與品質邊界不應進入 repo 工具第一版。 |
...
這個工具不自動呼叫外部 LLM，不自動分派 agent，不修改 prompt，不修改 schema，不寫入 Google Sheets。`prepare` 產生的 `expert-prompts/` 只是任務提示；操作者必須自行分派審查者，並要求審查者在 `expert-reviews/` 記錄 provenance。
