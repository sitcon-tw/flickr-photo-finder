...
這份 brief 依據 `docs/public-frontend-agent-research.md` 的代理使用者研究建立。它不是最終 UI 設計稿，而是下一輪 GitHub Pages 前端重構的產品與工程需求。實作前仍應先檢查目前 `app/` 程式碼、資料量與部署設定。

## 目標

把目前欄位導向的 MVP 前端，重構成面向大量照片的找圖工作台。使用者應能從工作任務出發，在上千到上萬張照片中快速得到可信候選，並能把候選交給其他人討論或使用。
...
完整篩選仍沿用 schema/taxonomy，但第一版必須補足目前 MVP 缺口：
