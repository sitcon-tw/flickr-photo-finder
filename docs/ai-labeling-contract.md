...
3. 技術志工或 agent 已執行 `pnpm ai:prepare`，產生 `tmp/ai-runs/<run-id>/`；若是多模型或多輪比較，也可再用 `pnpm eval:attempt` 從該 run 建立 attempt 目錄。
...
```bash
pnpm eval:search -- --run-dir tmp/ai-runs/<run-or-attempt>
```
