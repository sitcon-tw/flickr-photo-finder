...
這裡討論的不是另建後台或資料庫軟體，而是照片索引資料的權威來源、驗證方式、協作流程、agent 維護方式與未來演進邊界。
...
## 未來是否需要離開 Sheets-first

導入後台或另一套正式資料庫不是目前專案目標。只有當資料量、協作人數、權限分層或查詢需求明顯超過 Google Sheets-first 架構能合理承受的範圍時，才重新評估。
...
出現以下情況時，再認真評估是否需要 Google Sheets 之外的正式資料層：
...
在這些情況出現前，1.0 的重點仍應放在資料品質、標籤是否符合真實需求，以及志工是否能持續維護。

## 目前結論

1.0 階段應把 Google Sheets 視為正式照片索引，把 repo 視為治理與工具層。
