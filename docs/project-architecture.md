...
目前 repo 已有本機相簿盤點 CLI、`fixtures/albums.csv` fixture 格式、可回寫 Google Sheets `albums` 的 CSV 產生流程、從正式 Sheets 匯出工作 CSV 的 SDK 工具、從匯出 `albums.csv` 或直接從 Sheets API 列出與篩選相簿的 CLI、互動式選擇單本相簿的 CLI、從選定相簿產生 intake run artifact 的流程、以官方 Google Sheets API SDK 套用初始化 CSV 與 intake run artifact 的 dry-run/write 工具、AI 初標候選 metadata 的 prepare/review/diff/plan/report/apply 流程、`eval:attempt` / `eval:sample` / `eval:search` 模型品質評估工具、GitHub Pages artifact build/deploy workflow，以及 Apps Script 維護輔助 source。GitHub Pages 已設定為 GitHub Actions artifact 部署，`master` push 或手動觸發 workflow 會 build/check/deploy 公開檢索頁。Apps Script 已可透過 `clasp` 綁定到 Sheet UI 的 bound script，並提供 Sheet-bound 校對 sidebar、header notes、單值下拉選單、`schema_meta`、`validation_report` 與基本驗證；實際部署仍需由有目標 Sheet / Apps Script 權限的維護者執行。`pnpm workflow` 目前已包裝常見相簿匯入、AI prepare/review/report、Sheets 維護與 Pages artifact build/check 流程；模型品質評估工具屬於特別工作，保留為 `eval:*` 低階指令；更細的 `ai:validate`、`ai:diff`、`ai:plan`、`album:add`、`photo:add` 與 `photos:import` 也保留為低階指令。
