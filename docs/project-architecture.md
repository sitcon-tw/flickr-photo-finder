...
目前 repo 已有本機相簿盤點 CLI、`fixtures/albums.csv` fixture 格式、可回寫 Google Sheets `albums` 的 CSV 產生流程、從正式 Sheets 匯出工作 CSV 的 SDK 工具、從匯出 `albums.csv` 列出與篩選相簿的 CLI、互動式選擇單本相簿的 CLI、從選定相簿產生 intake run artifact 的流程，以及以官方 Google Sheets API SDK 套用初始化 CSV 與 intake run artifact 的 dry-run/write 工具；尚未完成的是讓選擇流程直接讀正式 Sheets API 而不需要先匯出工作 CSV，以及把 AI 輔助、Apps Script、GitHub Pages 部署串成完整使用流程。
