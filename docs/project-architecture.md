...
目前 repo 已有本機相簿盤點 CLI、`data/albums.csv` fixture 格式、可回寫 Google Sheets `albums` 的 CSV 產生流程，以及從選定相簿產生 intake run artifact 的流程；尚未完成的是以官方 Google Sheets API SDK 直接寫回正式 Google Sheets，以及由正式 `albums` 觸發完整匯入。
