...
- 前端會追蹤文字搜尋、篩選、任務模式切換、零結果、載入更多、照片選擇、開啟 Flickr 原始頁、下載大圖、開啟原圖尺寸頁、複製 Flickr 連結、複製 finder 連結與候選清單操作。
...
| `zero_results` | 自訂事件 | 搜尋或篩選後沒有任何結果。 | 找出 taxonomy、描述或欄位覆蓋缺口。 |
...
在有搜尋字串、篩選器或任務模式時，若結果為 0 才送出。事件應去除重複狀態，避免同一個零結果條件連續送出。
