...
- service account 已可依 `docs/ga4-operations.md` 的 API Explorer workaround 加入 GA4 property；提供 GA4 property ID 後，`pnpm analytics:dimensions:sync -- --property <GA4_PROPERTY_ID> --write` 已可同步低基數 custom dimensions。
...
7. 依 `docs/ga4-operations.md` 將 service account 加入 GA4 property，並用 `pnpm analytics:dimensions:check -- --property <GA4_PROPERTY_ID>` / `pnpm analytics:dimensions:sync -- --property <GA4_PROPERTY_ID> --write` 管理低基數 custom dimensions。已完成初步後台同步。
