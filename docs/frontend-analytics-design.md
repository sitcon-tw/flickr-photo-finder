...
這份文件定義 GitHub Pages 前端若導入 GA4 時，應該量測什麼、不要量測什麼，以及後續如何把行為訊號轉成照片索引維護工作。
...
- 照片卡片會追蹤文字搜尋、篩選、照片選擇、開啟 Flickr 原始頁、開啟大圖、開啟原圖尺寸頁與複製 finder 連結。
- 卡片會提供 `Flickr`、`大圖`、`原圖` 與 `複製連結` 操作。
- `複製連結` 複製的是 finder deep link，用於討論欄位、標籤與使用判斷是否合適，不是複製 Flickr URL。

評估前應重新檢查，避免把過期狀態當成事實：

```bash
rg -n "gtag|G-|google analytics|GA4|analytics|dataLayer|clipboard|copy_finder_link|open_image_size|open_flickr|measurement" app docs data config scripts README.md package.json
...
2. 哪些搜尋沒有結果，或結果太少？
3. 哪些篩選欄位真的被使用？
4. 哪些照片雖被點擊、開啟來源或複製 credit，卻仍是 `unreviewed`、`ai_labeled` 或 `needs_review`？
5. 哪些照片接近被實際使用？
...
- GA4 custom dimensions 只註冊低基數參數，例如 `result_count_bucket`、`search_surface`、`public_use_status`、`curation_status`、`recommended_use`、`sponsorship_filter_used`。
...
5. 新增 `複製連結` 操作，並以 `copy_finder_link` 追蹤 finder deep link 複製成功。已完成。
6. 使用 GA4 DebugView 或 Realtime 確認事件名稱、參數與 no-op 行為。待有 GA4 管理權限者驗證。
7. 執行 `pnpm pages:build` 與 `pnpm pages:check`，確認 GitHub Pages artifact 包含需要的設定但不包含 secret。每次前端變更後都應執行。
...
- 零結果或低結果數搜尋。
- 被使用的篩選條件。
- 被點擊、開啟 Flickr、開啟大圖、開啟原圖尺寸頁或複製 finder 連結的 `photo_id`。
...
3. 經常被開啟 Flickr、開啟原圖尺寸頁或複製 finder 連結的照片，檢查欄位、標籤、`photographer`、`license` 與 `public_use_status` 是否足夠。
