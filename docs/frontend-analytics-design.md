...
- service account 已可依 `docs/ga4-operations.md` 的 API Explorer workaround 加入 GA4 property；提供 GA4 property ID 後，`pnpm ga4:dimensions:sync -- --property <GA4_PROPERTY_ID> --write` 已可同步低基數 custom dimensions。
...
- GA4 custom dimensions 只註冊低基數參數；註冊清單以 `config/ga4-custom-dimensions.json` 為準，例如 `result_count_bucket`、`search_surface`、`sort_mode`、`public_use_status`、`curation_status`、`recommended_use`、`sponsorship_filter_used`。
...
7. 依 `docs/ga4-operations.md` 將 service account 加入 GA4 property，並用 `pnpm ga4:dimensions:check -- --property <GA4_PROPERTY_ID>` / `pnpm ga4:dimensions:sync -- --property <GA4_PROPERTY_ID> --write` 管理低基數 custom dimensions。已完成初步後台同步。
