...
截至 React final cutover，這份文件保留 GA4 事件設計與 legacy vanilla 實作紀錄，但正式 React Pages artifact 尚未重新接上 GA4 dispatch。若要恢復使用行為分析，應以本文件的產品問題與 privacy 邊界為基礎，重新評估事件 taxonomy，再於 React UI 中集中實作，不直接假設 legacy `app/main.js` 的事件覆蓋仍在 production 生效。
...
- legacy `app/analytics.js` 與 `app/main.js` 曾在 vanilla artifact 中動態載入 Google tag 並送出下列事件；React cutover 後它們不是 production artifact 入口。
- React artifact 若重新導入 analytics，應保留「不送完整 prompt、完整 filter JSON、自由文字需求或候選清單內容」的 privacy 邊界。
...
```bash
rg -n "gtag|G-|google analytics|GA4|analytics|dataLayer|clipboard|copy_flickr_link|copy_finder_link|download_image_size|open_image_size|open_flickr|measurement" app-react app-core app docs data config scripts README.md package.json
sed -n '1,220p' config/project.json
sed -n '1,220p' app-react/index.html
sed -n '1,220p' app-react/src/App.tsx
pnpm finder:build
pnpm finder:check
```
...
2. legacy vanilla analytics helper 曾集中處理 `gtag` 載入狀態、事件送出、字串清理、低基數 bucket 與 no-op；React artifact 若恢復 GA4，需重新接到 React state 與 component action。
3. React artifact 恢復 analytics 前，先重新決定是否沿用下方事件名稱，或建立新 taxonomy 並在本文件記錄遷移。
4. 更新前端測試或 smoke script，至少確保 analytics helper 在沒有 GA4 ID 時 no-op，不阻擋前端載入。
5. 依 `docs/ga4-operations.md` 將 service account 加入 GA4 property，並用 `pnpm analytics:dimensions:check` / `pnpm analytics:dimensions:sync -- --write` 管理低基數 custom dimensions。已完成初步後台同步。
6. 執行 `pnpm finder:build` 與 `pnpm finder:check`，確認 GitHub Pages artifact 包含需要的設定但不包含 secret。每次前端變更後都應執行。
