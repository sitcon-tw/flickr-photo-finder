...
截至 2026-05-11，repo 內已加入 GA4 基礎追蹤、任務模式、照片卡片操作、候選清單與大量結果瀏覽事件。
...
Pages Finder 遷移到 React 後，既有 GA4 event names、parameters 與 custom dimensions 不作為相容性限制。過去 GA4 紀錄尚未進入正式分析，因此 React 版可以重新定義事件分類；但仍需遵守本文件的隱私、低基數、no credential 與 GA4 collection limit 邊界。

React 版第一階段事件採用 `finder_*` 命名，先追蹤核心找圖流程：

| 事件 | 觸發時機 |
| --- | --- |
| `finder_task_select` | 使用者切換任務模式。 |
| `finder_photo_preview` | 使用者開啟照片詳情。 |
| `finder_candidate_add` | 使用者加入候選。 |
| `finder_candidate_remove` | 使用者移除候選。 |
| `finder_candidate_copy` | 使用者成功複製候選清單。 |
| `finder_ai_prompt_copy` | 使用者成功複製 AI 助手提示詞。 |
| `finder_ai_sheet_open` | 使用者從 AI 助手區開啟正式 Sheets。 |

這些事件不送完整 query string、完整 filter JSON、候選清單內容或提示詞內容。若後續需要分析搜尋與篩選，應先重新設計低基數參數與 custom dimensions，再更新 `config/ga4-custom-dimensions.json`。

React 版互動事件應帶 `surface` 低基數參數，值為 `desktop` 或 `mobile`。這讓後續分析能分辨桌面工作台與手機快速挑選的流程差異，而不需要從 viewport、URL 或裝置資訊推測。
...
| `select_task_mode` | 自訂事件 | 使用者切換社群、網站、贊助、新聞稿等任務模式。 | 觀察哪種工作情境最常被拿來找圖。 |
...
| 參數 | 說明 |
| --- | --- |
| `task_mode` | 使用者切換後的任務模式，例如 `social`、`hero`、`sponsor-report`。 |
...
| 參數 | 說明 |
| --- | --- |
| `task_mode` | 使用者當下的任務模式。 |
| `has_search_term` | 觸發事件時是否有文字搜尋。 |
| `has_filters` | 觸發事件時是否有任一非任務、非搜尋篩選。 |

### `zero_results`

在有搜尋字串、篩選器、排序或任務模式時，若結果為 0 才送出。事件應去除重複狀態，避免同一個零結果條件連續送出。
...
3. React 版第一階段先追蹤 `finder_task_select`、`finder_photo_preview`、`finder_candidate_add`、`finder_candidate_remove`、`finder_candidate_copy`、`finder_ai_prompt_copy` 與 `finder_ai_sheet_open`。已完成。
4. 搜尋、篩選、load more、source action 與 zero result 事件尚未在 React 版重新導入；不得用 vanilla 事件清單驗收 React 版。若要恢復，需先重新設計低基數參數與 custom dimensions。
5. 使用 GA4 DebugView 或 Realtime 確認 React 事件名稱、`surface`、`copy_template`、no-op 與隱私行為。已完成初步串接；後續前端事件調整仍應重新驗證。
6. 對外圖片來源操作目前以 UI 行為保留，analytics 事件另行設計，不沿用 vanilla event names。
...
- 任務模式切換。
- 照片 preview/detail 開啟。
- 候選加入、移除與複製。
- AI 助手提示詞複製與 Sheets 開啟。
- 這些互動當時的 `public_use_status` 與 `curation_status` 分布。

有限制：

- React 版目前不送 `photo_id`、`content_id`、`search_term`、`result_rank`、完整篩選條件或 result count；目前不應期待 GA4 UI 直接提供逐張照片排行榜、原始搜尋字串彙整或零結果分析。
- `recommended_use`、`result_count_bucket`、`search_surface` 等 dimensions 仍保留給後續事件設計；沒有對應事件前不應解讀為現行 React 報表欄位。
- 若要分析互動照片本身用途，需回到 Google Sheets join、BigQuery raw events，或未來另外新增低基數事件參數。
