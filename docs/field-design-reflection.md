...
這份文件記錄實際 AI 標記後，回頭檢視 photo schema、taxonomy 與 prompt 是否符合 SITCON 找照片需求的觀察。

第一次接手專案時，請先讀 `docs/README.md` 的「先建立共同語言」與「整體資料生命週期」。本文引用的 AI run 是歷史 AI 標記工作包與比較結果，用來反思欄位設計，不是正式 Sheets 的完整資料狀態。
...
| `ai-prepare-2026-05-09T12-15-05-642Z-attempt-claude-r1` | 2025 SITCON Hour of Code 桃園場 | 122 | 兒童工作坊、報到互動、手作物件、場地導引、使用提醒語意 |
...
- `55247911036`、`55247933626`：講者與背板同框。適合講者宣傳、新聞稿或活動回顧，但 `safe_crop`、`recommended_uses` 與 reason 語言品質需要人工把關。
...
- `54977003667`：報到桌與多名參與者互動。它同時涉及人數、工作人員、交流與活動流程。
...
- AI 不應把照片標成 `approved`。
- SITCON Flickr 照片本身已經是經同意釋出的公開來源；這個欄位不是同意、授權或公開 / 非公開判斷。
- 空白不代表 approved，只代表 AI 沒提出使用品質或整理提醒。
- `needs_review` 是提醒，不是排除，也不應因人物類型或可識別細節而自動填入。

大量 run 顯示，模型容易把 `needs_review` 當成保守預設，也可能完全省略這個 optional 欄位。未來 UI 與 AI 消費資料時，應把 `public_use_status` 解讀為使用提醒；空白不代表 approved，`needs_review` 也不代表照片未取得公開同意。

### 不導入人臉辨識與自動人名標註

曾有回饋提到，若像 Google Photos 或 Immich 一樣做人臉辨識、人物聚類，或讓 AI 標出可篩選的人名，找知名講者、來賓或「大人物」照片會更方便。這個需求合理，但目前專案選擇不導入這類能力；這不是尚未實作的缺口，而是刻意保留的產品邊界。

不同視角的評估如下：

| 視角 | 評估 |
| --- | --- |
| 找圖工作流程 | 本專案要解決的是社群、網站、贊助、新聞稿與活動回顧等工作情境找圖，不是建立人物相簿。知名人物照片通常有明確活動時間、議程或相簿脈絡；在 Flickr 既有時間排序、相簿、活動 metadata、`scene_tags`、`people_count` 與 `visual_description` 的輔助下，通常能用較低風險方式找到候選照片。 |
| 隱私與個資 | 人臉辨識與人物聚類會處理可識別個人的生物特徵。即使照片來源本來公開，將臉轉成可搜尋的人物索引，仍會改變資料使用方式與被搜尋者的感受。 |
| 人名標註 | 人名欄位不一定使用生物特徵，但一旦把姓名和照片列綁在一起，就會讓「找某個人的所有照片」變得容易。這和單純保存活動照片不同，應被視為另一種身份索引風險。 |
| 肖像權與社群信任 | 宣傳、招商或成果報告越接近商業使用，就越需要避免把非工作人員或未明確同意的人物拿來替活動背書。便利搜尋不能蓋過被拍攝者對曝光脈絡的合理期待。 |
| 技術與維護 | 導入 face embedding、人物群組、人工合併/拆分、錯誤更正、刪除請求與權限控管，會把專案推向個人資料管理系統。這超出目前「Flickr 上的公開照片索引層」定位，也會增加維護者和整理者的責任。 |

因此目前採用的替代方案是：

- 保留 `people_count`、`subject_type = people`、`scene_tags`、`visual_description`、`public_use_status` 與 `safe_crop`，讓系統能描述人物是否出現、人數規模、場景、動作、裁切風險與使用提醒。
- 不自動辨識人物是誰，不建立 face embedding、face cluster、person id 或自動人名欄位。
- 不要求 AI 從照片推測姓名、身份、單位或角色；若照片中清楚可見文字，應以「畫面可見文字」描述，而不是轉成身份判斷。
- 若未來確實需要支援講者或工作人員搜尋，應優先使用活動議程、相簿時間、已公開的 session metadata、人工整理的素材包或明確同意範圍內的公開欄位；這應被視為新的資料治理設計，不是把 AI 看臉結果直接寫進 `photos`。

這個邊界也代表：改善搜尋品質時，應優先強化 `visual_description`、活動脈絡、素材包、用途與場景欄位，而不是把「能不能任意找到某個人」當成預設能力。
...
| `child` / `student` | 不放 | 這是人物身份或活動場景脈絡，不是主體類型；應由 `scene_tags` 或 `visual_description` 處理。 |
