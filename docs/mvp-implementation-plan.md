...
- 先從 100 到 300 張高機率會被使用或已被使用者提供相簿的照片開始。
- 來源可以是近年常用活動、過去社群與網站素材、行銷組贊助提案常用照片、各組人工推薦照片，或使用者明確提供的 Flickr 相簿。
...
目前 repo 內的 `data/` 應視為 MVP sample、local fixture 與 Sheets 匯出格式參考，不是正式資料來源。正式資料以 Google Sheets 為準；repo 負責保存 schema、taxonomy、validation、匯入工具、AI/agent 維護脈絡與可重跑流程。協作原則記錄在 `docs/database-collaboration-strategy.md`，正式 Sheets 表格設計記錄在 `docs/google-sheets-database-design.md`。

目前 repo 內先保留三份資料：

- `data/photos.csv`: 照片索引 sample 與匯出格式參考。
...
2. 選一批 100 到 300 張候選照片或相簿匯入資料建立第一版公開索引。
