...
目前 repo 內的 `data/` 應視為 MVP sample、local fixture 與 Sheets 匯出格式參考，不是正式資料來源。正式資料以 Google Sheets 為準；repo 負責保存 schema、taxonomy、validation、匯入工具、AI/agent 維護脈絡與可重跑流程。協作原則記錄在 `docs/database-collaboration-strategy.md`。
...
| `curation_status` | 列舉值 | `unreviewed`、`ai_labeled`、`reviewed`、`featured`、`archived`。 |
