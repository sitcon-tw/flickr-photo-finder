...
實際欄位順序、型別、必填條件、reviewed 完整度與 approved 使用要求，以 `data/photo-schema.json` 為準；人工填寫規則請看 `docs/data-entry-guide.md`，欄位速查請看 `docs/photo-fields-reference.md`。這份 MVP 計畫只保留欄位分工與驗證方向，不另外維護一份欄位清單。
