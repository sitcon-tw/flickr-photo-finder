...
第一版以 Google Sheets 作為正式照片索引與人工維護介面。CSV 與 JSON 只作為匯出、公開前端與工具交換格式，不是第二份正式資料來源。
...
SITCON 2026 年會已經結束，因此 `data/sponsorship-items.json` 應視為固定版本資料，不需要建立自動同步流程。未來年度的贊助徵求資料可能完全取代 2026 CFS snapshot，也可能根據 2026 欄位設計再次精進；屆時應明確建立對應年度或來源的版本資料，而不是假設 2026 資料會持續更新。

概念上，每張照片資料會分成幾組責任：

- Flickr 與匯入工具可取得的來源資料，例如照片 ID、原始連結、縮圖、相簿關係與可辨識的相簿脈絡。
- 人類整理者需要判斷的找圖 metadata，例如場景、主體、情緒、推薦用途、贊助品項、可裁切性、素材包、使用提醒與整理狀態。
...
2. 由各組提供已知常用或高價值照片。
3. 對候選照片做第一輪快速標註。
4. 把照片分成 `unreviewed`、`ai_labeled` 或 `reviewed`，並另外用 `public_use_status` 表達不建議推薦或仍需整理判斷的使用提醒。
5. 用真實任務測試搜尋，例如「找 5 張志工招募照片」、「找 10 張攤位贊助照片」、「找 3 張適合網站橫幅的橫式照片」。
6. 記錄找不到、難判斷、標籤不自然、欄位不足的情況。
7. 回頭修正欄位、標籤字典與素材包。

AI 可以用來產生標記候選，但不能取代人工審核。尤其是 `public_use_status`、`sponsorship_items`、`sponsorship_tags`、`recommended_uses` 這類欄位，牽涉使用品質、贊助脈絡或用途判斷，應由熟悉 SITCON 語境的人確認。
...
- 網站或設計夥伴能否找到橫式、有留白、適合做網站橫幅、社群版面或背景素材的照片。
...
- 使用者的自然語言需求是否沒有對應標籤。
- 搜尋結果是否太多、太少或排序不合理。
- `public_use_status` 是否標示不建議推薦，或授權資訊是否需要回 Flickr 原頁確認。
...
接下來的重點不是再決定是否需要 AI 標記或 Flickr 匯入腳本，而是改善已存在流程：
