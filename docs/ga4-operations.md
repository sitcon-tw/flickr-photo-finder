...
2. 切換任務模式，確認收到 `select_task_mode` 與 `task_mode`。
3. 輸入非空搜尋字串，確認收到 `search`。
4. 調整篩選器或排序，確認收到 `filter_results`；至少驗收一次活動/相簿篩選，確認 `album_filter_used = true`。
5. 使用活動/相簿、素材包、贊助品項、構圖或狀態篩選時，確認對應低基數參數有送出，不要把 `album_id`、相簿名稱或完整搜尋條件註冊成 custom dimension。
6. 點載入更多，確認收到 `load_more_results`。
7. 產生一次零結果，確認收到 `zero_results`。
8. 點擊照片圖片，確認收到 `select_content` 與 `open_flickr_source`。
9. 點 `下載大圖`，確認收到 `download_image_size` 且 `image_size = large_1024`。
10. 點 `原圖`，確認收到 `open_image_size` 且 `image_size = original`。
11. 點 `Flickr 連結` 與 `本頁連結`，確認分別收到 `copy_flickr_link`、`copy_finder_link`。
12. 加入與移出候選，確認收到 `add_candidate`、`remove_candidate`；複製候選清單後確認收到 `copy_candidate_list`。
13. 使用 AI 助手找圖入口，確認開啟 Sheets 收到 `open_sheets_for_ai_assistant`，複製提示詞收到 `copy_ai_assistant_prompt`。
...
- Values: `Event count`
- Filter: `Event name` matches `open_flickr_source|download_image_size|open_image_size|copy_flickr_link|copy_finder_link`
...
- Values: `Event count`
- Filter: `Event name` matches `add_candidate|remove_candidate|copy_candidate_list|open_sheets_for_ai_assistant|copy_ai_assistant_prompt`
