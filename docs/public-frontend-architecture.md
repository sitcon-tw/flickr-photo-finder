...
公開前端的長期拆分原則是 Functional Core / Product UI Shell，但既有 pure modules 不是不可破壞契約。搜尋、排序、URL state、資料正規化、候選輸出與 AI 助手提示詞等純資料邏輯若仍符合 React 狀態模型，應保留或 port 成可測試核心；若舊模組妨礙型別、安全性或 UX，應改寫。遷移期間若仍需維護 `app/` 原生 ES modules，應把它視為過渡層，不再擴張自製 select、sheet、popover 或 gesture 邏輯。
