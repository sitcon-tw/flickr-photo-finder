...
前端不應依賴 Sheets 的顏色、註解或篩選檢視。所有可搜尋、可篩選、可提醒的語意都應來自欄位值；來源列順序只作為同年份照片沿用 Flickr 相簿順序的穩定次序。
...
- `app/controls.js` 負責查詢 DOM controls/elements、建立可搜尋 multi-select / token autocomplete、填入篩選選項、優先檢視按鈕、優先檢視感知篩選分層與 active filter entry。控制項狀態仍由 `main.js` 的 finder state 組合進 render loop。
...
- filter、優先檢視控制項、URL key、狀態排序或跨介面 field set：再讀 `docs/shared-value-governance.md`，並從 `data/interface-registry.json` 修改 source of truth。
- GA4 event、custom dimension 或分析流程：再讀 `docs/frontend-analytics-design.md` 與 `docs/ga4-operations.md`。
- 搜尋信任邊界與空結果風險：讀 `docs/adr/0007-finder-index-results-are-not-absence-proof.md`；目前實作規格以本文件為準。
...
| filter、優先檢視控制項、URL key 與跨介面欄位集合 | 本文件、`docs/shared-value-governance.md`、`data/interface-registry.json` | `app/controls.js`、`app/url-state.js`、`app/task-modes.js` | `pnpm finder:test`、`pnpm shared-values:check` |
...
- 優先檢視控制項 defaults 與 page size 等 registry fallback：`app/task-modes.js`。
...
頁首提供 Flickr 來源與 GitHub 專案連結，網址由 `config/project.json` 設定。GitHub 連結供使用者了解專案或回報問題。
...
面對上千或上萬張照片時，公開前端應以「工作需求」作為初始心智模型，而不是只提供欄位表單。優先檢視可以調整推薦排序權重，但不應隱藏資料；使用者仍可透過活動/相簿、構圖、留白、裁切、場景、用途、贊助品項與素材包等欄位自行收斂結果。結果狀態列應清楚說明優先檢視是排序情境，不是硬篩選。
...
候選清單只存在瀏覽器當下狀態與 URL query，不寫回 Google Sheets。清單畫面應提供縮圖，避免只用 photo id 要求使用者記憶照片；照片卡片與預覽 dialog 都可以切換候選狀態，但同一張照片只能在候選清單中出現一次。預設複製動作應複製可重現目前 Finder 狀態的 canonical URL，包含優先檢視、搜尋、篩選、排序與 `selected` 候選照片，讓接收者打開後直接看到同一批候選。其他文字輸出格式應放在複製按鈕旁的 action menu，點選後直接複製，不保留持久選取狀態。桌面版可用 split button 節省空間；手機版應顯示 `其他格式` 與 `複製連結` 兩個並排按鈕，並把格式清單展開在候選 bottom sheet 內，避免 dropdown 超出畫面。預設 IM 討論版應只保留編號與 Flickr URL，方便使用者在聊天工具中回覆「選第幾張」，必要時才附上簡短使用提醒。協作檢查版則應包含整批 Finder 清單連結、Google Sheets 列連結、Flickr URL、整理狀態與使用提醒，服務資料維護與跨角色檢查。純 Flickr URL 版應只輸出每張照片的 Flickr URL，方便工具或文件貼上。
...
- 優先檢視對 `recommended_uses`、`mood_tags`、`scene_tags`、`sponsorship_tags`、`orientation`、`safe_crop`、`has_negative_space` 的權重。
...
- 依相簿、年份或工作建立二級 index，避免每次搜尋掃描全 index。
