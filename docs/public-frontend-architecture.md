...
Pages frontend 已是多人長期使用的主要產品介面，不再以手刻原生 DOM 互動作為長期方向。後續前端工具鏈應採 Vite + React + TypeScript，並以 React Aria Components 作為 dialog、popover、select / combobox、focus management 與 accessibility 的基礎 primitive。導入這些工具的目標是降低 mobile overlay、浮層定位、鍵盤焦點、scroll lock、觸控手勢與長清單操作反覆修補的風險；不是引入有強烈視覺風格的 UI kit。完整遷移基準、可破壞舊契約與 commit plan 記錄在 `docs/public-frontend-react-migration-plan.md`。
...
公開前端的產品 shell 已切換為 `app-react/` 的 Vite + React + TypeScript app。`app/index.html`、`app/main.js` 與 `app/styles.css` 這組 vanilla DOM surface 已退役，不再作為正式 Pages、dev server 或新功能入口。

目前拆分原則是 Functional Core / React Product UI Shell：

- `app-react/` 負責產品 UI、React state、桌面/手機 layout、React Aria dialog / select / popover、analytics 呼叫與使用者互動。
- `app/` 只保留 React 仍共用或尚待 port 的資料核心與 helper。這些模組可以繼續被測試引用，但不應新增 vanilla DOM 產品畫面。
- 搜尋、排序、URL state、資料正規化、候選輸出與 AI 助手提示詞等純資料邏輯若仍符合 React 狀態模型，可以暫留 `app/`；若舊模組妨礙型別、安全性或 UX，應 port 到 React / TypeScript domain。

目前仍被 React 或測試引用的核心包含：

- `app/search-sort.js`：search text、篩選、scoring、推薦排序與探索排序；不得直接讀 DOM 或全域控制項。
- `app/url-state.js`：URL query encode/decode；selected ids、filters 與 sort deep link 行為應先在這裡調整，直到 port 成 typed React domain。
- `app/ai-assistant.js`：AI 助手提示詞的純資料組裝，不處理 clipboard 或 DOM。
- `app/candidates.js`：候選清單資料選取與 copy text shaping；DOM render helper 只屬於待移除 legacy 區塊，不應再擴張。
- `app/data-loader.js`：讀取 project config、schema、taxonomy、search aliases 與 `photos` CSV，並依 schema 正規化 list 欄位、sheet row number 與 `search_text`。
- `app/controls.js`：React 目前仍引用 filter definitions 與相簿選項 helper；DOM 查詢與 vanilla control helper 是待 port legacy 區塊，不應再新增行為。
- `app/photo-render.js`：React 目前仍引用連結與標題 helper；DOM card render helper 是待 port legacy 區塊，不應再新增行為。
- `app/result-render.js`：保留可測試結果文字 helper；DOM result render helper 是待 port legacy 區塊。
- `app/overview-render.js`：保留舊統計邏輯參考；新 UI 應優先在 React component 或 typed domain 補上需要的統計。
...
若未來公開 CSV 體積或瀏覽器記憶體成為瓶頸，再優先調整公開前端資料載入方式：
