...
公開前端 1.0 直接讀取 Google Sheets `photos` 工作表的公開 CSV 輸出，不保存 secret，也不使用需要私人 credential 的 Google API。
...
- 未來可在 build 階段加入靜態 JSON、搜尋索引或 metadata 產生，而不改變 Pages 的安全邊界。

代價：

- 部署流程依賴 GitHub Actions 與 Pages artifact 設定。
- build script 與 check script 必須隨前端檔案拆分同步維護。
- 若 Google Sheets 公開 CSV 延遲或格式改變，前端仍會受影響。
