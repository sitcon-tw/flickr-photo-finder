...
- `pnpm ai:codex:meter -- --run-dir <dir> --session <codex-session> --mark-start/--mark-end`，記錄 Codex 操作者或 parent orchestration 的 token snapshot；大型 worker 可搭配 `ai:shard:log --codex-session <id>`，讓 `ai:bulk:status` 彙總 shown tokens、cached input、output 與 reasoning。
