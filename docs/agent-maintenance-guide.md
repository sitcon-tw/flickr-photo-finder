...
- 擁有 Google Sheets、Google API、OAuth、rclone 或 Apps Script credential。
...
1. 先確認需要哪一類資產或權限，例如 Google Sheets 編輯權限、Google Sheets API OAuth credential、Apps Script/clasp、rclone 或 AI API。
...
- rclone 若被使用，應限於 Drive 檔案搬運、備份或組織既有工作流，不作為 Sheets tab/range 寫入的主要流程。
