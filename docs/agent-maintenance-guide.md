...
3. Agent 或模型逐張輸出 `photo-artifacts/<photo_id>.json`，看完一張就落盤一張，再用 `pnpm ai:artifacts:merge -- --run-dir <dir>` 合併 `metadata-proposals.json`、`visual-inspection-audit.json` 與 `artifact-manifest.json`；大型 run 的 worker 也只輸出自己的 per-photo artifacts。不要改 `photos.json`、Sheets export 或正式 Google Sheets。大型 run 可先用 `pnpm ai:shard:prepare` 把分片中間檔放在 `/tmp/ai-labeling-shards/<run-id>/`，再用 `pnpm ai:shard:merge` 從 artifacts 合併；不要手動沿用先前 proposal。若 parent agent 具備建立 sub-agents 或 worker agents 的能力，smoke test 通過後應分派多個 worker 平行處理不同 shard，parent agent 只負責 orchestration、artifact 合併、validate、review 與修補。
