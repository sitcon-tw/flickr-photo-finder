...
若任務是替 `tmp/ai-runs/<run-id>/` 或 attempt 目錄裡的照片產生初標 metadata，請先讀 `docs/ai-labeling-operator-guide.md` 與 `docs/ai-labeling-contract.md`。前者是 prepare-to-review 的操作指南，後者是 AI 初標階段的輸入、圖片來源、`metadata-proposals.json` 輸出格式與驗證命令的合約。若任務是評估模型輸出品質、調整 prompt 或判斷 `visual_description` 是否有搜尋增益，請再讀 `docs/ai-labeling-evaluation-notes.md`。

建議流程：

1. 先用 `pnpm ai:prepare` 建立 run；同一批輸入要交給多模型或多輪時，用 `pnpm ai:attempt` 建立 attempt，不要手動複製資料夾。
...
6. 若本次重點是 `visual_description` 或自然語言找圖，先用 `pnpm search:experimental -- --run-dir <dir>` 比較 taxonomy-only baseline 與 description 搜尋結果。
