...
封存 tag `archive/pages-react-aria-migration-prototype-2026-05-14` 只作 reference，不 cherry-pick prototype 實作 commit。後續每個 phase 都必須先定義目標、不改什麼、驗收 gate 與 rollback 條件。
...
目的：正式 Pages artifact 切到 React app，但保留 vanilla surface 以便 rollback。
...
- Production P0 hotfixes may still land on vanilla UI until final cutover.
