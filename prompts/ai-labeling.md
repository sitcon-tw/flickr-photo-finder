...
接著再判斷 `scene_tags`、`mood_tags`、`recommended_uses`、`public_use_status` 與其他欄位。`recommended_uses` 不是必填欄位；只有照片明確適合某個工作用途時才輸出。不要把 `活動回顧` 或 `社群貼文` 當成預設用途。請先找比 `活動回顧` 更有區辨度的 1 到 3 個用途，例如講者、志工、新聞、網站、簡報或贊助相關用途。只有其他用途都不適用，但這張照片仍有明確一般回顧價值時，才可單獨使用 `活動回顧`，且 reason 必須說明這張照片的具體回顧價值。
